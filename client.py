...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.btn = CTkButton(self, text='▶️', command=self.toggle_show_menu, width=30, fg_color="red")
        self.btn.place(x=0, y=0)
        # main
        self.chat_field = CTkTextbox(self, font=('Arial', 14, 'bold'), state='disable', fg_color="#99675D")
        self.chat_field.place(x=0, y=0)
        self.message_entry = CTkEntry(self, placeholder_text='Введіть повідомлення:', height=40)
        self.message_entry.place(x=0, y=0)
        self.send_button = CTkButton(self, text='>', width=50, height=40, command=self.send_message, fg_color="red")
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.entry.insert(0, self.username)   # показати поточне ім’я
            self.entry.pack()
